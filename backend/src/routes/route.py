...
from models.route import Leg, Results, TripPattern
...
async def change_bike_station(data: BikeStationData):
    ...
    new_legs: List[Leg] = []
    new_pattern: TripPattern = {"legs": [], "aimedEndTime": ""}
    ...
    speed = route_data.bike_average_speed if route_data.use_own_bike else route_data.bikesharing_average_speed
    ...
    async with Client(transport=transport) as session:
        ...
        if route_data.arrive_by:
            ...
            if data.origin_bike_station:
                ...
                bike_pattern = await walk_bicycle_route(f"{data.bike_stations[data.new_index]["place"]["latitude"]}, {data.bike_stations[data.new_index]["place"]["longitude"]}", f"{legs[i]["toPlace"]["latitude"]}, {legs[i]["toPlace"]["longitude"]}", time_to_depart, "bicycle", speed, session)
                ...
                if waypoint_found:
                    walk_pattern = await walk_bicycle_route(f"{data.bike_stations[data.new_index]["place"]["latitude"]} ,{data.bike_stations[data.new_index]["place"]["longitude"]}", f"{legs[leg_index]["fromPlace"]["latitude"]}, {legs[leg_index]["fromPlace"]["longitude"]}", time_to_depart, "foot", route_data.walk_average_speed, session)
            ...
            # The destination bike station is changed
                ...
                walk_pattern = await walk_bicycle_route(f"{data.bike_stations[data.new_index]["place"]["latitude"]} ,{data.bike_stations[data.new_index]["place"]["longitude"]}", f"{legs[i]["toPlace"]["latitude"]}, {legs[i]["toPlace"]["longitude"]}", time_to_depart, "foot", route_data.walk_average_speed, session)
                ...
                if "bikeStationInfo" in new_legs[0]:
                    # Route bicycle segment previous waypoint to new destination station
                    bike_pattern = await walk_bicycle_route(f"{legs[leg_index]["fromPlace"]["latitude"]}, {legs[leg_index]["fromPlace"]["longitude"]}", f"{new_legs[0]["bikeStationInfo"]["latitude"]}, {new_legs[0]["bikeStationInfo"]["longitude"]}", time_to_depart, "bicycle", speed, session)
        ...
        # Departure based routing
            ...
            if data.origin_bike_station:
                ...
                walk_pattern = await walk_bicycle_route(f"{legs[i]["fromPlace"]["latitude"]}, {legs[i]["fromPlace"]["longitude"]}", f"{data.bike_stations[data.new_index]["place"]["latitude"]} ,{data.bike_stations[data.new_index]["place"]["longitude"]}", time_to_depart, "foot", route_data.walk_average_speed, session)
                ...
                # Reconnect modified route to original waypoints
                leg_index = i + 2
                if "bikeStationInfo" in new_legs[-1]:
                    # Compute route from new origin station to next point
                    bike_pattern = await walk_bicycle_route(f"{new_legs[-1]["bikeStationInfo"]["latitude"]}, {new_legs[-1]["bikeStationInfo"]["longitude"]}", f"{legs[leg_index]["toPlace"]["latitude"]}, {legs[leg_index]["toPlace"]["longitude"]}", time_to_depart, "bicycle", speed, session)
            ...
            # The destination bike station is changed
                ...
                # Compute new bicycle route from selected bike station
                bike_pattern = await walk_bicycle_route(f"{legs[i]["fromPlace"]["latitude"]}, {legs[i]["fromPlace"]["longitude"]}", f"{data.bike_stations[data.new_index]["place"]["latitude"]} ,{data.bike_stations[data.new_index]["place"]["longitude"]}", time_to_depart, "bicycle", speed, session)
                ...
                if "bikeStationInfo" in new_legs[-1]:
                    ...
                    if waypoint_found:
                        walk_pattern = await walk_bicycle_route(f"{new_legs[-1]["bikeStationInfo"]["latitude"]}, {new_legs[-1]["bikeStationInfo"]["longitude"]}", f"{legs[leg_index]["toPlace"]["latitude"]}, {legs[leg_index]["toPlace"]["longitude"]}", time_to_depart, "foot", route_data.walk_average_speed, session)
                        new_legs.extend(walk_pattern[0]["legs"])
