...
async def walk_bicycle_route(origin: str, destination: str, time_to_depart: str, mode: str, session): 
...
async def public_transport_route(waypoints: List[str], time_to_depart: str, arrive_by: bool, max_transfers: int, modes: List[str], session, num_of_waypoints: int, add_direct_mode: bool = False):
    ...
    async def public_route(local_variables, time_to_depart):
    ...
    trip_patterns = []
    indicies = list(reversed(range(len(waypoints) - 1))) if arrive_by else range(len(waypoints) - 1)
    ...
    shape_id_to_data = {}
    async def fetch_shape(shape_id):
    ...
    for pattern in trip_patterns:
        for leg in pattern["legs"]:
            if leg["mode"] in ["bus", "tram", "rail", "metro", "water", "trolleybus"]:
                ...
                start_index = quay_index_map.get(leg["fromPlace"]["name"], -1)
                ...
                stop_index_map = {stop["stop_name"]: i for i, stop in enumerate(shape_by_ID["stops"])}
                from_idx = stop_index_map.get(leg["fromPlace"]["name"], -1)
                to_idx = stop_index_map.get(leg["toPlace"]["name"], -1)
