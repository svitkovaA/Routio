    ...
    if i == 0 and distance * 1.2 <= max_bike_distance:
        ...
        if extra_distance <= 1 or distance_to_next <= 1:
            ...
            if extra_distance <= 1:
                ...
                if use_own_bike:
                    ...
                    for pattern in result:
                        # Invalid result
                        if not pattern.legs[1].fromPlace:
                            continue

                        # Compute walk segment from origin to bicycle station
                        walk_result = await walk_bicycle_route(
                            bike_group[0], 
                            f"{pattern.legs[1].fromPlace.latitude}, {pattern.legs[1].fromPlace.longitude}", 
            ...
            # Distance to next is less than 1km (second algorithm case)
                ...
                for pattern in result:
                    # Validate result
                    fromPlace = pattern.legs[leg_index].fromPlace
            ...
            # Route using public transport
                ...
                pt_result = await process_public_route(
                    waypoints[:i + 1], 
                    result[0].legs[0].aimedStartTime if arrive_by else time_to_depart, 
                    arrive_by, 
                    max_transfers, 
                    modes, 
                    walk_speed, 
            ...
            for pattern in result:
                ...
                tasks.append(
                    process_public_route(
                        ...
                        pattern.legs[0].aimedStartTime if arrive_by else time_to_depart, 
                        arrive_by, 
                        max_transfers, 
                        modes, 
                        walk_speed, 
